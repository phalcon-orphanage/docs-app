...
Usually the Flash Messaging service is requested from the services container. If you're using [Phalcon\Di\FactoryDefault](api/Phalcon_Di) then [Phalcon\Flash\Direct](api/Phalcon_Flash) is automatically registered as `flash` service and [Phalcon\Flash\Session](api/Phalcon_Flash) is automatically registered as `flashSession` service. You can also manually register it:
