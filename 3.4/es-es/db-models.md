...
| `columns`     | Retorna las columnas especificadas en vez de todas las columnas del modelo. When using this option an incomplete object is returned                                                                                                                 | `'columns' => 'id, name'`                                         |
...
All the queries are internally handled as [PHQL](/3.4/en/db-phql) queries. PHQL es un lenguaje de alto nivel, orientado a objetos y similar a SQL. Este lenguaje le proporciona más características para realizar consultas como unir otros modelos, definir grupos, agregar agregaciones, etcétera.
