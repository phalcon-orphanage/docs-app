...
[Phalcon\Http\Request](api/Phalcon_Http_Request) allows you to access the values stored in the `$_REQUEST`, `$_GET` and `$_POST` arrays and sanitize or filter them with the [filter](/3.4/en/filter) service, (by default [Phalcon\Filter](api/Phalcon_Filter)). Los siguientes ejemplos ofrecen el mismo comportamiento:
