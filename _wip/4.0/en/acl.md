...
[Phalcon\Acl](api/Phalcon_Acl) provides an easy and lightweight management of ACLs as well as the permissions attached to them. [Access Control Lists](http://en.wikipedia.org/wiki/Access_control_list) (ACL) allow an application to control access to its areas and the underlying objects from requests. 

In short, ACLs have two objects: The object that needs access, and the object that we need access to. In the programming world, these are usually referred to as Roles and Resources. In the Phalcon world, we use the terminology `Subject` and `Operation`.

{: .alert .alert-info}**Use Case**

{: .alert .alert-info}An accounting application needs to have different groups of users have access to various areas of the application.

{: .alert .alert-info}| **Operation**                | **Subject**   |
{: .alert .alert-info}|------------------------------|---------------|
{: .alert .alert-info}| Administrator Access         | Login page    |
{: .alert .alert-info}| Accounting Department Access | Admin page    |
{: .alert .alert-info}| Manager Access               | Invoices page |
{: .alert .alert-info}| Guest Access                 | Reports page  |

</h5>
  
As seen above in the use case, an `Operation` is defined as who needs to access a particular `Subject` i.e. an area of the application. A `Subject` is defined as the area of the application that needs to be accessed. 
...
Phalcon also offers the [Phalcon\Acl\AdapterInterface](api/Phalcon_Acl_AdapterInterface), offering an easy way for developers to build their own adapters.
...
The default action is **`Phalcon\Acl::DENY`** for any `Operation` or `Subject`. This is on purpose to ensure that only the developer or application allows access to specific subjects and not the ACL component itself.
...
## Adding Operations
As mentioned above, an `Operation` is an object that can or cannot access a set of `Subjects` in the access list. To see this in action, using the example outlined above, we will add the relevant `Operations` and `Subjects` in our list:
