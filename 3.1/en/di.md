...
<a name='envents'></a>
### Events
[Phalcon\Di](api/Phalcon_Di) is able to send events to an [EventsManager](/3.1/en/events) if it is present. Events are triggered using the type 'di'. Some events when returning boolean false could stop the active operation.
