...
This component can be used in a plain HTML+PHP view or in a [Volt](/3.1/en/volt) template.
...
Phalcon also provides a [form builder](/3.1/en/forms) to create forms in an object-oriented manner.
...
## Tag Service
[Phalcon\Tag](api/Phalcon_Tag) is available via the [tag](/3.1/en/tag) service, this means you can access it from any part of the application where the services container is located:
...
Then change the definition of the service [tag](/3.1/en/tag):
...
You can also check out [Volt](/3.1/en/volt) a faster template engine for PHP, where you can use a more developer friendly syntax for helpers provided by [Phalcon\Tag](api/Phalcon_Tag).
