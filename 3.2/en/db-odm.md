...
In addition to its ability to [map tables](/3.2/en/models) in relational databases, Phalcon can map documents from NoSQL databases. The ODM offers a CRUD functionality, events, validations among other services.
...
Additionally, this component is integrated with the [Phalcon Events Manager](/3.2/en/events) ([Phalcon\Events\Manager](api/Phalcon_Events_Manager)), this means we can create listeners that run when an event is triggered.
...
<h5 class='alert alert-warning' markdown='1'>For more information on validators, see the [Validation documentation](/3.2/en/validation) </h5>
